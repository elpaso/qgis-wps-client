...
from wpslib.processdescription import getFileExtension, isMimeTypeVector, isMimeTypeRaster, isMimeTypeText, isMimeTypeFile, isMimeTypePlaylist, isMimeTypeOWS, StringInput, TextInput, SelectionInput, VectorInput, MultipleVectorInput, RasterInput, MultipleRasterInput, ExtentInput, CrsInput, VectorOutput, RasterOutput
...
# self.dlg.bookmarksChanged.connect()"), self,
# SIGNAL("bookmarksChanged()"))
    ...
    def createProcessGUI(self):
        ...
        # Generate the editable outpt widgets, you can set the output to none
        ...
        self.addOkCancelButtons(
            self.dlgProcess,  self.dlgProcessTabFrameLayout)
        ...
        for input in self.process.inputs:
            ...
            if inputType == VectorInput or inputType == MultipleVectorInput:
                ...
                if inputType == VectorInput:
                    self.complexInputComboBoxList.append(self.tools.addComplexInputComboBox(title, inputIdentifier, str(
                        complexDataFormat), layerNamesList, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout))
                else:
                    self.complexInputListWidgetList.append(self.tools.addComplexInputListWidget(title, inputIdentifier, str(
                        complexDataFormat), layerNamesList, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout))
            elif inputType == StringInput:
                dValue = input.defaultValue
                self.literalInputLineEditList.append(self.tools.addLiteralLineEdit(
                    title, inputIdentifier, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout, dValue))
            elif inputType == TextInput:
                complexDataFormat = input.dataFormat
                if isMimeTypePlaylist(complexDataFormat["MimeType"]) is not None:
                    self.inputDataTypeList[inputIdentifier] = complexDataFormat
                    # Playlist (text) inputs
                    self.complexInputTextBoxList.append(self.tools.addComplexInputTextBox(
                        title, inputIdentifier, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout, str(complexDataFormat)))
                else:
                    self.complexInputTextBoxList.append(self.tools.addComplexInputTextBox(
                        title, inputIdentifier, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout))
                ...
                self.literalInputComboBoxList.append(self.tools.addLiteralComboBox(
                    title, inputIdentifier, valList, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout))
            elif inputType == ExtentInput:
                myExtent = pystring(
                    self.iface.mapCanvas().extent()).replace(':', ',')
                self.bboxInputLineEditList.append(self.tools.addLiteralLineEdit(
                    title + "(minx,miny,maxx,maxy)", inputIdentifier, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout, myExtent))
...
#                self.literalInputComboBoxList.append(self.tools.addLiteralComboBox("Supported CRS", inputIdentifier, crsListe, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout))
        ...
        self.tools.addCheckBox(QCoreApplication.translate("QgsWps", "Process selected objects only"), QCoreApplication.translate(
            "QgsWps", "Selected"),  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout)
    ...
    def defineProcess(self):
        ...
        if len(self.process.inputs) > 0:
            ...
            for textBox in self.complexInputTextBoxList:
                ...
                if isMimeTypePlaylist(self.mimeType) is not None:
                    ...
                    ), self.mimeType, schema, encoding, textBox.document().toPlainText())

                else:  # It's not a playlist
                    request.addPlainTextInput(
                        textBox.objectName(), textBox.document().toPlainText())
...
#              QMessageBox.information(None, '',  pystring(comboBox.objectName()))
                ...
                encoding = self.inputDataTypeList[pystring(comboBox.objectName())][
                    "Encoding"]
                self.myLayer = self.tools.getVLayer(comboBox.currentText())

#              try:
                if isMimeTypeVector(self.mimeType) is not None and encoding != "base64":
                ...
                elif isMimeTypeVector(self.mimeType) is not None or isMimeTypeRaster(self.mimeType) is not None:
                    request.addGeometryBase64Input(comboBox.objectName(
                    ), self.mimeType, self.tools.getVLayer(comboBox.currentText()))
#              except:
...
#                  return
            ...
            for listWidgets in self.complexInputListWidgetList:
                ...
                schema = self.inputDataTypeList[listWidgets.objectName()][
                    "Schema"]
                encoding = self.inputDataTypeList[
                ...
                for i in range(listWidgets.count()):
                    ...
                    if isMimeTypeVector(self.mimeType) is not None and self.mimeType == "text/xml":
                        ...
                        request.addMultipleGeometryInput(listWidgets.objectName(
                        ), self.mimeType, schema, encoding, gmldata, useSelected)
                    elif isMimeTypeVector(self.mimeType) is not None or isMimeTypeRaster(self.mimeType) is not None:
                        request.addMultipleGeometryBase64Input(listWidgets.objectName(
                        ), self.mimeType, self.tools.getVLayer(listWidget.text()))
            ...
            for comboBox in self.literalInputComboBoxList:
                ...
                request.addLiteralDataInput(
                    comboBox.objectName(), comboBox.currentText())
            ...
            for lineEdit in self.literalInputLineEditList:
                ...
                request.addLiteralDataInput(
                    lineEdit.objectName(), lineEdit.text())
        ...
        if dataOutputs.size() > 0 or len(self.complexOutputComboBoxList) > 0:
            ...
            for i in range(dataOutputs.size()):
                ...
                outputIdentifier = pystring(f_element.elementsByTagName(
            ...
            for comboBox in self.complexOutputComboBoxList:
                ...
                schema = self.outputDataTypeList[
                    pystring(outputIdentifier)]["Schema"]
                encoding = self.outputDataTypeList[
                    pystring(outputIdentifier)]["Encoding"]

                request.addReferenceOutput(
                    outputIdentifier, self.mimeType, schema, encoding)
        ...
        self.wps = ExecutionResult(self.getLiteralResult, self.getResultFile,
                                   self.successResult, self.errorResult, self.streamingHandler,  self.progressBar)
...
##############################################################################
    ...
    def streamingHandler(self, encoding, playlistUrl):
        ...
        chunks = 0
        if isMimeTypeVector(self.mimeType) is not None:
            for lineEdit in self.literalInputLineEditList:
        ...
        # Streaming handler
        self.dataStream = Streaming(
            self, self.iface, chunks, playlistUrl, self.mimeType, encoding)
 ...
 ##############################################################################
    ...
    def loadData(self,  resultFile):
        ...
        layerName = self.tools.uniqueLayerName(
            self.processIdentifier + "_" + self.identifier)
       ...
       # Raster data
        ...
        # Text data
            ...
            # TODO: this should be handled in a separate diaqgswps.pylog to
            # save the text output as file'
            QApplication.restoreOverrideCursor()
            text = open(resultFile, 'r').read()
            # TODO: This should be a text dialog with safe option
            self.tools.popUpMessageBox(QCoreApplication.translate(
                "QgsWps", 'Process result (text/plain)'), text)

        # Everything else
        elif isMimeTypeFile(self.mimeType) is not None:
            # TODO: this should be handled in a separate diaqgswps.pylog to
            # save the text output as file'
            QApplication.restoreOverrideCursor()
            text = open(resultFile, 'r').read()
        ...
        # Everything else
            ...
            self.tools.popUpMessageBox(QCoreApplication.translate(
                "QgsWps", 'Process result (unsupported mime type)'), content)

        if not bLoaded:
            QMessageBox.information(self.iface.mainWindow(),
                                    QApplication.translate(
                                        "QgsWps", "Result not loaded to the map"),
    ...
    def getResultFile(self, identifier, mimeType, encoding, schema, reply):
        ...
        resultFile = self.wps.handleEncoded(
            tmpFile, mimeType, encoding,  schema)
...
##############################################################################
    ...
    def errorResult(self, exceptionHtml):
        ...
        QMessageBox.critical(self.iface.mainWindow(),
                             "Exception report", exceptionHtml)
