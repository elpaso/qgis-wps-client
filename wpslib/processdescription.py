...
RASTER_MIMETYPES = [{"MIMETYPE": "image/tiff", "GDALID": "GTiff", "EXTENSION": "tif"},
                    ...
                    {"MIMETYPE": "image/geotiff",
                     "GDALID": "GTiff", "EXTENSION": "tif"},
                    {"MIMETYPE": "application/x-erdas-hfa",
                     "GDALID": "HFA", "EXTENSION": ""},
                    {"MIMETYPE": "application/netcdf",
                     "GDALID": "netCDF", "EXTENSION": ""},
                    ...
                    {"MIMETYPE": "application/x-geotiff",
                     "GDALID": "GTiff", "EXTENSION": "tif"},
                    {"MIMETYPE": "application/x-esri-ascii-grid",
                     "GDALID": "AAIGrid", "EXTENSION": "asc"},
                    {"MIMETYPE": "application/image-ascii-grass", "GDALID": "GRASSASCIIGrid", "EXTENSION": "asc"}]

OWS_MIMETYPES = [
    {"MIMETYPE": "application/x-ogc-wms", "PROVIDER": "wms", "TYPE": "RASTER"},
    {"MIMETYPE": "application/x-ogc-wfs",
     "PROVIDER": "wfs", "TYPE": "VECTOR"},
]
...
VECTOR_MIMETYPES = [{"MIMETYPE": "application/x-zipped-shp", "SCHEMA": "", "GDALID": "ESRI Shapefile", "DATATYPE": "SHP", "EXTENSION": "zip"},
                    {"MIMETYPE": "application/vnd.google-earth.kml+xml", "SCHEMA": "KML",
                        "GDALID": "KML", "DATATYPE": "KML", "EXTENSION": "kml"},
                    {"MIMETYPE": "text/xml", "SCHEMA": "GML", "GDALID": "GML",
                        "DATATYPE": "GML", "EXTENSION": "gml"},
                    {"MIMETYPE": "text/xml; subtype=gml/2.", "SCHEMA": "GML2",
                     "GDALID": "GML", "DATATYPE": "GML2", "EXTENSION": "gml"},
                    {"MIMETYPE": "text/xml; subtype=gml/3.", "SCHEMA": "GML3",
                     "GDALID": "GML", "DATATYPE": "GML3", "EXTENSION": "gml"},
                    {"MIMETYPE": "application/json", "SCHEMA": "JSON",
                     "GDALID": "GEOJSON", "DATATYPE": "JSON", "EXTENSION": "json"},
                    {"MIMETYPE": "application/geojson", "SCHEMA": "GEOJSON", "GDALID": "GEOJSON", "DATATYPE": "GEOJSON", "EXTENSION": "geojson"}]
# mimeTypes for streaming
PLAYLIST_MIMETYPES = [{"MIMETYPE": "application/x-ogc-playlist+",
                       "SCHEMA": "", "GDALID": "", "DATATYPE": "PLAYLIST", "EXTENSION": "txt"}]
...
def getIdentifierTitleAbstractFromElement(element):
    ...
    title = pystring(getOwsElement(element, "Title").at(
...
def isMimeTypeOWS(mimeType, ignorePlaylist=False):
    """Check for OWS input"""
    if not ignorePlaylist:
        if isMimeTypePlaylist(mimeType) != None:
            return None

    for rasterType in OWS_MIMETYPES:
        if rasterType["MIMETYPE"] in mimeType.lower():
            return rasterType
    return None
...
def getBaseMimeType(dataType):
    # Return a base mimeType (might not be completed) from a data type
    # (e.g.GML2)
    for vectorType in VECTOR_MIMETYPES:
...
def allowedValues(aValues):
    ...
    if v_range_element.size() > 0:
        min_val = getOwsElement(value_element, "MinimumValue").at(
            0).toElement().text()
        max_val = getOwsElement(value_element, "MaximumValue").at(
            0).toElement().text()
        ...
        try:
            ...
            QMessageBox.critical(None, QApplication.translate(
                "QgsWps", "Error"), QApplication.translate("QgsWps", "Maximum allowed Value is too large"))
...
StringInput = namedtuple(
    'StringInput', 'identifier title minOccurs defaultValue')
TextInput = namedtuple('TextInput', 'identifier title minOccurs dataFormat')
SelectionInput = namedtuple(
    'SelectionInput', 'identifier title, minOccurs valList')
VectorInput = namedtuple(
    'VectorInput', 'identifier title minOccurs dataFormat')
MultipleVectorInput = namedtuple(
    'MultipleVectorInput', 'identifier title minOccurs dataFormat')
RasterInput = namedtuple(
    'RasterInput', 'identifier title minOccurs dataFormat')
MultipleRasterInput = namedtuple(
    'MultipleRasterInput', 'identifier title minOccurs dataFormat')
FileInput = namedtuple('FileInput', 'identifier title minOccurs dataFormat')
MultipleFileInput = namedtuple(
    'MultipleFileInput', 'identifier title minOccurs dataFormat')
...
class ProcessDescription(QObject):
    ...
    def requestUrl(self):
        ...
        if '?' in self.server.baseUrl:
            request = "&Request=DescribeProcess&identifier=" + \
                self.identifier + "&Service=WPS&Version=" + self.version
        else:
            request = "?Request=DescribeProcess&identifier=" + \
                self.identifier + "&Service=WPS&Version=" + self.version
    ...
    def _parseProcessXML(self):
        ...
        self.processIdentifier = pystring(processDescription.at(0).toElement().elementsByTagNameNS(
            "http://www.opengis.net/ows/1.1", "Identifier").at(0).toElement().text()).strip()
        self.processName = pystring(processDescription.at(0).toElement().elementsByTagNameNS(
            "http://www.opengis.net/ows/1.1", "Title").at(0).toElement().text()).strip()

        self.identifier, self.title, self.abstract = getIdentifierTitleAbstractFromElement(
            self.doc)
        self.inputs = []
        self.outputs = []
    ...
    def _parseProcessInputs(self):
        ...
        for i in range(dataInputs.size()):
            ...
            inputIdentifier, title, abstract = getIdentifierTitleAbstractFromElement(
                f_element)
            minOccurs = int(f_element.attribute("minOccurs", "1"))
            maxOccurs = int(f_element.attribute("maxOccurs", "1"))

            # Iterate over all complex inputs and add combo boxes, text boxes
            # or list widgets
            ...
            if complexData.size() > 0:
                ...
                supportedComplexDataFormat = getSupportedMimeTypes(
                    complexDataTypeElement)
                complexDataFormat = getDefaultMimeType(complexDataTypeElement)

                # Store the input formats
                self._inputsMetaInfo[
                    inputIdentifier] = supportedComplexDataFormat
                ...
                if isMimeTypeVector(complexDataFormat["MimeType"]) != None:
                    ...
                    complexDataFormat = self.getSupportedGMLDataFormat(
                        inputIdentifier)
                    ...
                    # Vector inputs
                    if maxOccurs == 1:
                        self.inputs.append(VectorInput(
                            inputIdentifier, title, minOccurs, complexDataFormat))
                    else:
                        self.inputs.append(MultipleVectorInput(
                            inputIdentifier, title, minOccurs, complexDataFormat))
                elif isMimeTypeText(complexDataFormat["MimeType"]) != None:
                    # Text inputs
                    self.inputs.append(
                        TextInput(inputIdentifier, title, minOccurs, complexDataFormat))
                elif isMimeTypeRaster(complexDataFormat["MimeType"]) != None:

                    # Raster inputs
                    if maxOccurs == 1:
                        self.inputs.append(RasterInput(
                            inputIdentifier, title, minOccurs, complexDataFormat))
                    else:
                        self.inputs.append(MultipleRasterInput(
                            inputIdentifier, title, minOccurs, complexDataFormat))
                    ...
                    if maxOccurs == 1:
                        self.inputs.append(
                            FileInput(inputIdentifier, title, minOccurs, complexDataFormat))
                    else:
                        self.inputs.append(MultipleFileInput(
                            inputIdentifier, title, minOccurs, complexDataFormat))
        ...
        for i in range(dataInputs.size()):
            ...
            if literalData.size() > 0:
                ...
                if aValues.size() > 0:
                    ...
                    if len(valList) > 0:
                        if len(valList[0]) > 0:
                            self.inputs.append(SelectionInput(
                                inputIdentifier, title, minOccurs, valList))
                        else:
                            self.inputs.append(StringInput(
                                inputIdentifier, title, minOccurs, str(valList)))
                else:
                    self.inputs.append(StringInput(
                        inputIdentifier, title, minOccurs, dValue))
        ...
        for i in range(dataInputs.size()):
            ...
            if bBoxData.size() > 0:
                ...
                defaultCrsElement = bBoxElement.elementsByTagName(
                    "Default").at(0).toElement()
                defaultCrs = defaultCrsElement.elementsByTagName("CRS").at(
                    0).toElement().attributeNS("http://www.w3.org/1999/xlink", "href")
                crsListe.append(defaultCrs)

                self.inputs.append(ExtentInput(
                    inputIdentifier, title, minOccurs))

                supportedCrsElements = bBoxElement.elementsByTagName(
                    "Supported")

                for i in range(supportedCrsElements.size()):
                    crsListe.append(supportedCrsElements.at(i).toElement().elementsByTagName(
                        "CRS").at(0).toElement().attributeNS("http://www.w3.org/1999/xlink", "href"))

                self.inputs.append(
                    CrsInput(inputIdentifier, title, minOccurs, crsListe))
    ...
    def _parseProcessOutputs(self):
        ...
        for i in range(dataOutputs.size()):
            ...
            outputIdentifier, title, abstract = getIdentifierTitleAbstractFromElement(
                f_element)
            ...
            if complexOutput.size() > 0:
                ...
                complexOutputFormat = getDefaultMimeType(
                    complexOutputTypeElement)
                supportedcomplexOutputFormat = getSupportedMimeTypes(
                    complexOutputTypeElement)
                if isMimeTypeVector(complexOutputFormat["MimeType"]) != None:
                    self.outputs.append(VectorOutput(
                        outputIdentifier, title, complexOutputFormat))
                else:
                    self.outputs.append(RasterOutput(
                        outputIdentifier, title, complexOutputFormat))
    ...
    def getDataTypeInfo(self, mimeType, name):
        # Return a dict with mimeType, schema and encoding for the given
        # mimeType
