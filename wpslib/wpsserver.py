...
class WpsServer(QObject):
    ...
    def _capabilitiesRequestFinished(self):
        ...
        if self._theReply.error() == 1:
            QMessageBox.information(None, '', QApplication.translate(
                "QgsWpsGui", "Connection Refused. Please check your Proxy-Settings"))
            pass
        ...
        if version != "1.0.0":
            QMessageBox.information(None, QApplication.translate(
                "QgsWps", "Only WPS Version 1.0.0 is supported"), xmlString)
            pass
    ...
    def parseCapabilitiesXML(self):
        ...
        for i in range(identifier.size()):
            ...
            self.processes.append(ProcessDescription(
                self, str(i_element.text())))
